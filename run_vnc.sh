...
container=clever-sitl
image=sfalexrog/clever-sitl:vnc-gui
port=6080
extra_run_args=""
quiet=""
...
if [ "${os}" != "Linux" ]; then
	...
	if ! docker-machine inspect "${vm}" &> /dev/null; then
		...
		docker-machine -D create -d virtualbox --virtualbox-memory 4096 ${vm}
	fi
...
if [ "${os}" = "Linux" ] || [ "${os}" = "Darwin" ]; then
	mount_local=" -v ${pwd_dir}:/home/rosuser/work "
fi
port_arg=""
if [ -n "$port" ]; then
